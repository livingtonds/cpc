...
from utils.embeder import ModelEmbed, ModelEmbedSimple
...
class InferenceWrapperSimple():
    def __init__(self,
                 ...
                 vectore_storage_path='test.ann',
                 df_path="train_df.csv"):
        self.model_embd = ModelEmbedSimple(model=model,
                                           processor=processor,
                                           device=device)
    ...
    def get_res_by_two_images(self, img_1, img_2, k=2):
        ...
        concated_images = concat_two_images(img_1, img_2)
        embeding = self.model_embd.get_embd(concated_images)
        res_dict_from_bd = self.annoy_db.get_data_by_vector(embeding, k=k)
